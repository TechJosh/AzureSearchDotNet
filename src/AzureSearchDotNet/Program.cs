...
namespace AzureSearchDotNet
{
    class Program
    {
        ...
        static void Help()
        {
            var tbl = new ConsoleTable("Command", "Description");
            tbl.AddRow("Help", "Shows this table")
                .AddRow("Create", "Creates the search index")
                .AddRow("Load", "Loads the documents into the index")
                .AddRow("Weights", "Adds weight scoring to the index")
                .AddRow("Search", "Searches the index documents")
                .AddRow("Exit", "Quites this application")
        ...
        static void Weights()
        {
            ...
            var index = cli.Indexes.Get(indexName);

            ScoringProfile profile = index.ScoringProfiles.FirstOrDefault(sp => sp.Name == "Default");

            if (profile != null)
            {
                Console.WriteLine("{0}", "Deleting Scoring Profile...");
                index.ScoringProfiles.Remove(profile);
            }

            Console.WriteLine("{0}", "Creating Scoring Profile...");
            ...
            Console.WriteLine("{0}", "Setting Scoring Weights...");
            profile.TextWeights.Weights.Add("First", 10);
            profile.TextWeights.Weights.Add("Last", 5);
            profile.TextWeights.Weights.Add("Email", 1);

            Console.WriteLine("{0}", "Adding Scoring Profile...");
            index.ScoringProfiles.Add(profile);
            index.DefaultScoringProfile = "Default";

            Console.WriteLine("{0}", "Saving Scoring Profile...");
            cli.Indexes.CreateOrUpdate(index);

        }

        static void Load()
        {
            Console.WriteLine("{0}", "Setting up Data...");
            ...
            Console.WriteLine("{0}", "Preparing Upload Data...");
        ...
        static void Search()
        {
            ...
            Console.Write("Enter Search Term (use * for wildcard): ");
        ...
        {
            if (cli.Indexes.Exists(indexName))
            {
                Console.WriteLine("{0}", "Deleting index...\n");
                cli.Indexes.Delete(indexName);
            }
        }
        ...
        private static void CreateIndex(string indexName)
        {
            ...
            cli.Indexes.Create(definition);
        }

        private static void WriteDocuments(DocumentSearchResult<Person> searchResults)
        {
            var weighted = searchResults.Results.Select(x => WeightedPerson.Convert(x.Document, x.Score));
            ConsoleTable
                .From<WeightedPerson>(weighted)
                .Write();
    ...
    public class WeightedPerson: Person
    {
        public double Score { get; set; }

        public static WeightedPerson Convert(Person p, double score)
        {
            var w = new WeightedPerson
            {
                Id = p.Id,
                First = p.First,
                Last = p.Last,
